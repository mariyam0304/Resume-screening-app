...
const API = {
  ...
  listVideos: (cid) => api('/video/candidate/' + cid),
  uploadVideo: (cid, formData) => api('/video/upload/' + cid, { method: 'POST', body: formData }),
  deleteVideo: (vid) => api('/video/' + vid, { method: 'DELETE' }),
  rateVideo: (vid, rating) => api('/video/' + vid + '/rate', { method: 'POST', body: { rating } }),
  savePosition: (vid, position) => api('/video/' + vid + '/position', { method: 'POST', body: { position } }),
  listVideoNotes: (vid) => api('/video/' + vid + '/notes'),
  addVideoNote: (vid, b) => api('/video/' + vid + '/notes', { method: 'POST', body: b }),
  deleteVideoNote: (nid) => api('/video/notes/' + nid, { method: 'DELETE' }),
  videoStreamUrl: (vid) => API_BASE + '/video/' + vid + '/stream',
  videoDownloadUrl: (vid) => API_BASE + '/video/' + vid + '/download',
  updateWeights: (b) => api('/screening/weights', { method: 'POST', body: b }),
