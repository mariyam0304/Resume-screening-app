...
# ============================================================
# LIST VIDEOS FOR A CANDIDATE (owner only)
# ============================================================
@video_bp.route('/candidate/<int:cid>', methods=['GET'])
@login_required
def list_videos(cid):
    Candidate.query.filter((Candidate.id == cid) & _owner_filter()).first_or_404()
...
def upload_video(cid):
    ...
    c = Candidate.query.filter((Candidate.id == cid) & _owner_filter()).first_or_404()
...
def stream_video(vid):
    ...
    c = Candidate.query.filter((Candidate.id == v.candidate_id) & _owner_filter()).first()
    if not c:
        abort(404)
...
def rate_video(vid):
    ...
    v.rating = max(0, min(5, int(data.get('rating', 0))))
    db.session.commit()
...
def delete_note(nid):
    ...
    v = VideoInterview.query.get(n.video_id)
    if not v:
        abort(404)
    c = Candidate.query.filter((Candidate.id == v.candidate_id) & _owner_filter()).first()
    if not c:
