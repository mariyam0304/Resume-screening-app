const Router = {
  ...
  async handle() {
    ...
    console.log('[Router] Navigating to:', parsed.route);
    ...
    const fn = this.routes[parsed.route];
    const view = document.getElementById('view');

    if (!fn) {
      console.error('[Router] No route defined for:', parsed.route);
      view.innerHTML = '<div class="card"><h2>Unknown route: ' + parsed.route + '</h2></div>';
      return;
    }
    ...
    try {
      ...
      console.error('[Router] Error rendering', parsed.route, e);
      view.innerHTML =
        '<div class="card" style="border-left:5px solid var(--danger);">' +
          '<h2 style="color:#991b1b;">Error loading: ' + parsed.route + '</h2>' +
          '<p><strong>Message:</strong> ' + (e.message || e) + '</p>' +
          '<p class="muted">Press F12 → Console for full details.</p>' +
          '<pre style="background:#fee2e2;padding:10px;border-radius:6px;overflow:auto;font-size:12px;">' +
            (e.stack || '').toString().replace(/</g, '&lt;').slice(0, 800) +
          '</pre>' +
